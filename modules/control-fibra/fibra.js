...
window.FibraModule = (function(){
  const { escapeHtml, escapeAttr, capitalizeWords, todayStr, api, moduleIcon } = window.App3;
      ...
      <div class="modal">
        ...
        <div class="modal-body">
          ...
          <form id="form" style="display:none;">
            ...
            <div class="grid">
              <div class="field"><label>Fecha de ingreso <span class="req">*</span></label><input type="date" id="f_fechaIngreso"><div class="err" id="err_fechaIngreso"></div></div>
              ...
              <div class="field"><label>Fecha pactada</label><input type="date" id="f_fechaPactada"><div class="err" id="err_fechaPactada"></div></div>
              <div class="field"><label>Franja pactada</label>
                <select id="f_franjaPactada"><option value="AM">AM</option><option value="PM">PM</option></select>
                <div class="err"></div>
              </div>
              <div class="field"><label>Código de OT</label><input type="text" id="f_ot" class="mono"><div class="err"></div></div>
              <div class="field"><label>Código de SDS</label><input type="text" id="f_sds" class="mono"><div class="err"></div></div>
              <div class="field"><label>Fecha real de instalación</label><input type="date" id="f_fechaInstalacion"><div class="err" id="err_fechaInstalacion"></div></div>
            ...
            <div class="grid cols-3">
              <div class="field"><label>Localidad</label><input type="text" id="f_localidad" data-cap="words"><div class="err"></div></div>
              <div class="field"><label>Calle</label><input type="text" id="f_calle" data-cap="words"><div class="err"></div></div>
              <div class="field"><label>Altura</label><input type="text" id="f_altura" inputmode="numeric"><div class="err"></div></div>
              ...
              <div class="field"><label>Tipo</label>
                <select id="f_tipoDomicilio"><option>Casa</option><option>Edificio</option><option>Pasillo</option><option>Empresa</option></select>
                <div class="err"></div>
              </div>
            </div>
            ...
            <div class="grid">
              ...
              <div class="field"><label>DNI / CUIT</label><input type="text" id="f_dni" class="mono" inputmode="numeric"><div class="err" id="err_dni"></div></div>
              <div class="field"><label>Fecha de nacimiento</label><input type="date" id="f_fechaNacimiento"><div class="err"></div></div>
              <div class="field"><label>Mail</label><input type="text" id="f_email"><div class="err" id="err_email"></div></div>
              <div class="field"><label>Teléfono</label><input type="text" id="f_telefono" inputmode="tel"><div class="err" id="err_telefono"></div></div>
            ...
            <div class="grid">
              ...
              <div class="field"><label>Cantidad de TV</label>
                <select id="f_cantidadTV"><option value="0">N/A</option><option value="1">1</option><option value="2">2</option><option value="3">3</option></select>
                <div class="err"></div>
              </div>
            </div>
          </form>
  ...
  function mount(root, ctx){
    ...
    function populatePlanSelect(currentValue){
      ...
      const opts = ['<option value="">— Sin definir —</option>'];
    ...
    function clearForm(){
      for(const id of FIELD_IDS){
        const el = $('f_'+id);
        if(el) el.value = (id === 'cantidadTV') ? '0' : (id === 'franjaPactada') ? 'AM' : (id === 'estado') ? 'Falta pactar' : (id === 'tipoDomicilio') ? 'Casa' : (id === 'fechaIngreso') ? todayStr() : '';
      }
    ...
    function validate(data){
      ...
      setErr('fechaIngreso', !data.fechaIngreso ? 'Requerida' : '');
      setErr('clienteNombre', !data.clienteNombre ? 'Requerido' : '');
      setErr('fechaInstalacion', (data.estado === 'Instalada' && !data.fechaInstalacion) ? 'Requerida si el estado es Instalada' : '');
      setErr('fechaPactada', (data.estado === 'Pactada' && !data.fechaPactada) ? 'Requerida si el estado es Pactada' : '');
      if(data.dni){
