...
window.DashboardModule = (function(){
  ...
  async function loadWeekCal(root, goTo){
    ...
    const html = dias.map(d => {
      ...
      const ventasDelDia = ventas.filter(v => v.fechaPactada === iso && v.estado !== 'Instalada');
      const items = [
        ...
        ...ventasDelDia.map(v => `<div class="week-item venta" data-go="fibra" title="${escapeAttr(v.clienteNombre||'')}">📶 ${escapeHtml(v.clienteNombre||'Sin nombre')}</div>`),
